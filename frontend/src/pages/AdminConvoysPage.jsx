...
const AdminConvoysPage = () => {
  ...
  const fetchConvoys = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/convoys');
  ...
  const handleEditConvoy = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:5000/api/convoys/${editingConvoy.convoyId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteConvoy = async (convoyId) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/convoys/${convoyId}`, {
        method: 'DELETE'
      });
      const data = await response.json();
      if (data.success) {
